/*****************************************************************************
 * Copyright (C) vinit nirwane nirwanevv14.comp@coep.ac.in
 *
...
void home( ) {		//home page
	printf("\n\n\n\t\t\tBOOK MY SHOW- Movies, Entertainment & Fun \n");
		...
		if(flag == 0) 
			...
			switch(ch) {
				...
				case 3: u.a_id = 0;
					...
					break;
				case 4: printf("\n\nAccount Id:%ld\nName:%s\nemail:%s\nCredits earned:%d\nphone no:%lld", u.a_id, u.name, u.email, u.credits, u.p_no);
					fclose(fp);
					break;
				case 5: history(u); 
					fclose(fp); 
